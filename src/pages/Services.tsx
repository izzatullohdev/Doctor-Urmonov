  ...
  return (
    <div className=''>
      ...
      <div className="max-w-7xl mx-auto flex items-start justify-start max-lg:flex-col gap-10 mb-52 max-lg:px-4">
        ...
        {/* Chap qism: xizmat haqida to'liq */}
        <div className='w-[65%] max-lg:w-full bg-[#F5F8FF] rounded-[20px]'>
          {serviceData?.map((item) => (
            <div key={item.id} className="p-4">
              ...
              />
              <h1 title={item.title} className='text-[32px] font-montserrat font-medium text-[#0A0933] leading-[140%] mt-7 my-3'>{item.title}</h1>

              <div className="flex flex-col gap-2">
                {item.description?.map((desc, index) => (
                  <h3 key={index} title={desc} className='text-[#0A0933] text-[20px] leading-[140%] font-normal font-montserrat'>{desc}</h3>
                ))}
              </div>

              <div className="flex flex-col mt-2">
                {item.full_description?.map((desc, index) => (
                  <li key={index} title={desc} className='text-[#0A0933] text-[20px] leading-[140%] font-normal font-montserrat'>{desc}</li>
                ))}
              </div>
              ...
              {item.desc_title?.map((title, index) => (
                <div key={index} className="bg-[#F5F8FF] rounded-[10px] flex flex-col">
                  ...
                  {openIndex === index && item.desc?.[index] && (
                    <div className="px-5 pb-3 text-[16px] leading-[140%] font-montserrat font-normal">
                      {item.desc[index]}
                    </div>
                  )}
                </div>
